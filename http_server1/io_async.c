...
static void event_write_callback(event_loop_t* loop, usize transferred, void* user_data) {
	struct async_io* io = (struct async_io*)user_data;
...
u8 async_io_net_write(struct async_io* io, usize length, async_completion_handler_t write_cb) {
	if (io->transmission_buffer.size < length) return IO_ERROR_BUFFER_LESS_OR_NULL;
